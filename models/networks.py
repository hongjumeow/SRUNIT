...
def define_D(input_nc, ndf, netD, n_layers_D=3, norm='batch', init_type='normal', init_gain=0.02, no_antialias=False, gpu_ids=[], opt=None, noise=0.0):
...
class PatchSampleF(nn.Module):
    ...
    def forward(self, feats, num_patches=64, patch_ids=None, choice=None, use_mlp=True):
        return_ids = []
        return_feats = []
...
class ResnetGenerator(nn.Module):
    ...
    def forward(self, input=None, layers=[], encode_only=False, enc=False, dec=False, feats=[], noises=[]):
        if enc:
            fake = input
            for layer_id, layer in enumerate(self.model):
                fake = layer(fake)
...
class NLayerDiscriminator(nn.Module):
    ...
    def __init__(self, input_nc, ndf=64, n_layers=3, norm_layer=nn.BatchNorm2d, no_antialias=False, noise=0.0):
...
class FeatureDiscriminator(nn.Module):
    def __init__(self, input_nc=256, ndf=256):
